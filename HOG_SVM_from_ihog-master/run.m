...
C=0.0001;iter=1000;tol=0.001;
 [model, trainingAcc] = trainHOG(Xtrain', ytrain,sizeFeature, C,iter, tol);
 model.sbin=8;
 model.sb = 1; %Hyper param for scaling bias
 model.interval=1;
 model.thresh =0;
...
ypred = double(Xtest2 * model.wb > 0);
 ...
 positiveTestingAcc = sum((ytest ~= ypred )+ (ytest==1)==2);
 fprintf('Testing Accuracy  %g%%, positive Testing acc %g%%',testingAcc*100,positiveTestingAcc);
...
%  
 ...
 TestImage = double(imread('testImg.jpg'));
   [blobs,feat] = detect(TestImage,model,100000);  
 res = sub(blobs,~is_boxOutside(blobs,size(TestImage)));
 showBoxes(TestImage/255,res); drawnow;
 
 save beforHarden20131115;
 ...
 for ii=1:max_harden
     ...
     for i = 1: length(listing)
        filename = strcat(dirname,listing(i).name);
        im =  double(imread(filename))/255;    
        [blobs,dfeat] = detect(im,model,5000);  
...
ypred = double(Xtest2 * newmodel.wb > 0);
 ...
 testingAcc=(1- sum(ytest ~= ypred)/ntest);
 positiveTestingAcc = 1-(sum((ytest ~= ypred )+ (ytest==1)==2))/ntestpos;
...
% figure;showHOG(wShow);
 ...
 TestImage = double(imread('testImg.jpg'));
   [blobs,feat] = detect(TestImage,model,100000);  
% res = sub(blobs,~is_boxOutside(blobs,size(TestImage)));
 showBoxes(TestImage/255,blobs); drawnow;
 
 save afterHarden20131115;
